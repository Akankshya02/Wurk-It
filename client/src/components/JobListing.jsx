...
const JobListing = () => {
  ...
  return (
    <div className='container 2-xl:px-20 flex flex-col lg:flex-row max-lg:space-y-8 py-8'>
      {/* {Side Bar} */}
      <div className='w-full lg:w-1/4 bg-white-50 px-4'>
        {/* Search filter from hero component */}
        {
            isSearched && (searchFilter.title !== "" || searchFilter.location !== "") && (
                ...
                <div className='mb-4 text-gray-600'>
                    {searchFilter.title && (
                        <span className='inline-flex items-center gap-2.5 bg-yellow-50 border-yellow-7-- px-4 py-1.5 rounded'>
                            {searchFilter.title}
                            <img onClick={ e=> setSearchFilter(prev => ({...prev,title:""}))} src={assets.cross_icon} className='cursor-pointer'/>
                        </span>
                    )}
                     {searchFilter.location && (
                        <span className=' ml-2 inline-flex items-center gap-2.5 bg-yellow-100 border-yellow-7-- px-4 py-1.5 rounded'>
                            {searchFilter.location}
                ...
                filteredJobs.length > 0 && (
                    <div className='flex items-center justify-center space-x-2 mt-10'>
                        ...
                        {Array.from({length:Math.ceil(filteredJobs.length/6)}).map((_, index)=>(
                            <a href="#job-list">
                                <button onClick={()=> setCurrentPage(index+1)} className={`w-10 h-10 flex items-center justify-center border border-gray-300 rounded ${currentPage === index + 1 ? 'bg-yellow-500 text-white' : 'text-gray-500'}`}>{index + 1}</button>
                            </a>
                        ))}
                        <a href="#job-list">
